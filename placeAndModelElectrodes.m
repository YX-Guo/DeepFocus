...
for i = 1:length(elecPara) % size(elecLoc,1)    
    ...
    switch lower(elecPara(i).elecType)
        ...
        case 'disc'
            ...
            %MF 20200501 edit to allow custom orientation 
            if ~isempty(elecPara(i).elecOri) && any(elecPara(i).elecOri)
                normal=elecPara(i).elecOri./norm(elecPara(i).elecOri);
            end
...
%                 surf(elec_coor(:,1),elec_coor(:,2),elec_coor(:,3));
%                 surf(gel_coor(:,1),gel_coor(:,2),gel_coor(:,3));
            end
            
            gel_allCoord{i} = gel_coor; elec_allCoord{i} = elec_coor; % buffer for coordinates of each electrode and gel point
            %YG modified 01/14/24
            %{
            warning('YG modified to hardcode CPz elec & gel locations, for leadfield return')
            warning('Manually check elec placement!')
            if i == 2
                gel_allCoord{i} = load('example/BA25_scalp_elec.mat').gel_coor;
                elec_allCoord{i} = load('example/BA25_scalp_elec.mat').elec_coor;
            end
            %}
            % mod end

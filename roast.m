...
% DeepROAST modified ROAST original script to allow transnasal electrode
% placement 
% yuxinguo@andrew.cmu.edu
% March 21 2024
...
if any(~strcmpi(recipe,'leadfield'))
    ...
    if ~exist('elecOri','var')
        ...
        if ~iscellstr(elecType)
% MF remove 20200501 to allow custom orientation of disk electrodes
%             if ~strcmpi(elecType,'pad')
...
if isempty(Sopt)
    ...
    if all(isNew)
        ...
        %MF 20220904 add options to find in case more than one options file
        %matched
        load([dirname filesep Sopt(find(~isNew,1,'last')).name],'opt');
        ...
        MFi=input('Use new tag regardless? y/n','s');
        switch MFi
            case 'y'
                options.uniqueTag=simTag;
                options = writeRoastLog(subj,options,'roast');
            otherwise
...
if ~strcmp(baseFilename,'nyhead')
    ...
    %end
    disp('======================================================')
    disp('           Testing SPM12, Segment anyway ...          ')
    disp('======================================================')
    start_seg(subjRasRSPD,T2);
